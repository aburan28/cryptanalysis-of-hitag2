...
We first present the idea of the attack in the ideal situation. Later we build upon this idea by considering the practical problems and how they are mitigated in a more practical attack.
...
An important observation can be made from the figure: the number of computations in a chain is $(t-1)$, and not $t$ as we have been discussing. The same scene is observed in \cite{stamp2007acb} as well. With $(t-1)$ computations, $(t-1)$ keys exist in each chain. If the value of $t$ is large as compard to 1, then $(t-1) \approx t$. Hence in these references, the number of keys in every chain is assumed to be $t$ for all analysis. But we, on the other hand have assumed exactly $(t)$ keys per chain for all the analysis, and have implemented the same.
...
\end{tabular}
\end{center}
\caption{Performance of TMTO with Hellman tables}
